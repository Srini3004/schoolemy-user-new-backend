...
if (!process.env.EMAIL_FROM) {
  throw new Error("EMAIL_FROM is missing or not verified in SendGrid");
}
...
export const sendContactEmail = async (req, res) => {
  try {
    ...
    // Email to site admin
    const mailToAdmin = {
      from: { email: adminEmail, name: "Schoolemy Website" },
      to: adminEmail,
      subject: `Contact form: ${subject || "New message from website"}`,
      html: `
        <h3>New contact form submission</h3>
    ...
    // Acknowledge to user
    const mailToUser = {
      from: { email: adminEmail, name: "Schoolemy Support" },
      to: email,
      subject: `Thanks for contacting Schoolemy${subject ? ` — ${subject}` : ""}`,
      html: `
        <div style="font-family: Arial, sans-serif;">
          ...
          <p>Thanks for reaching out to Schoolemy. We have received your message and will get back to you shortly.</p>
          <p><strong>Your message:</strong></p>
          <div style="border-left:3px solid #eee;padding-left:8px;color:#333">
            ${message.replace(/\n/g, "<br/>")}
          </div>
          <hr/>
          <p style="color:#777;">© 2025 Schoolemy</p>
    ...
    console.error("Error in sendContactEmail:", error.response?.body || error);
