...
if (!process.env.EMAIL_FROM) {
  throw new Error("EMAIL_FROM missing or not verified");
}
...
export const sendNotification = async (userId, type, data) => {
  ...
  const emailTemplates = { /* ✅ KEEP YOUR EXISTING EMAIL HTML TEMPLATES */ };
  const smsTemplates = { /* ✅ KEEP YOUR EXISTING SMS TEMPLATES */ };
  const whatsappTemplates = { /* ✅ KEEP YOUR EXISTING WHATSAPP TEMPLATES */ };

  const emailHtml = emailTemplates[type]?.(user, data);
  const smsText = smsTemplates[type]?.(user, data);
  const whatsappText = whatsappTemplates[type]?.(user, data);

  if (!emailHtml || !smsText || !whatsappText) {
    ...
    try {
      await sgMail.send({
        ...
        from: {
          ...
          name: "Schoolemy Support",
        },
        subject:
          type === "welcome"
            ? `🎉 Welcome to ${data.courseName}!`
            : type === "reminder"
            ? `⏰ EMI Reminder – ${data.courseName}`
            : type === "late"
            ? `🚨 Overdue EMI – ${data.courseName}`
            : type === "lock"
            ? `🔒 Course Locked – ${data.courseName}`
            : `✅ Course Restored – ${data.courseName}`,
        html: emailHtml,
      });
